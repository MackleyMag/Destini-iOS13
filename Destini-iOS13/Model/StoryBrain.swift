...
struct StoryBrain {
    ...
    mutating func setChoiceMade(choice:String){
        
        choiceMade = nextStory(userChoice: choice)
    }
    
    func getStoryText() -> String {
        return stories[choiceMade].title
    }
    
    func getChoice(n: Int) -> String{
        if n == 1{
            return stories[choiceMade].choice1
        }else{
            return stories[choiceMade].choice2
        }
    }
    
    func nextStory(userChoice: String) -> Int{
        if userChoice == stories[choiceMade].choice1{
            return stories[choiceMade].choice1Destination
        }else{
            return stories[choiceMade].choice2Destination
        }
    }
}
